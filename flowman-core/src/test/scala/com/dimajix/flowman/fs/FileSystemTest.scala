...
class FileSystemTest extends AnyFlatSpec with Matchers with LocalSparkSession {
    ...
    "FileSystem.file" should "support resources somewhere via 'file(URI)'" in {
        val conf = spark.sparkContext.hadoopConfiguration
        val fs = FileSystem(conf)
        val file = fs.file(Resources.getURL("com/dimajix/flowman/some-test-resource.txt").toURI)
        ...
        val file2 = fs.file(Resources.getURL("com/dimajix/flowman/../flowman/some-test-resource.txt").toURI)
