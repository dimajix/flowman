...
class DatasetTemplateTest extends AnyFlatSpec with Matchers {
    "A DatasetTemplateInstance" should "be deserialized" in {
        val spec =
            """
              |kind: template/user
              |arg1: value_1
        ...
        val dataset = ObjectMapper.parse[DatasetSpec](spec)
        dataset shouldBe a[DatasetTemplateInstanceSpec]

        val datasetTemplate = dataset.asInstanceOf[DatasetTemplateInstanceSpec]
        datasetTemplate.args should be (Map("arg1" -> "value_1", "arg2" -> "value_2"))
    }
