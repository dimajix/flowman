...
class FileRelationTest extends FlatSpec with Matchers with LocalSparkSession {
    ...
    it should "support partitions" in {
        ...
        relation.exists(executor) should be (Yes)
        relation.exists(executor, Map()) should be (Yes)
        relation.exists(executor, Map("p_col" -> SingleValue("2"))) should be (Yes)
        relation.exists(executor, Map("p_col" -> SingleValue("3"))) should be (No)
        ...
        relation.exists(executor, Map()) should be (Yes)
        relation.exists(executor, Map("p_col" -> SingleValue("2"))) should be (Yes)
        relation.exists(executor, Map("p_col" -> SingleValue("3"))) should be (Yes)
