...
class RebalanceMappingTest extends AnyFlatSpec with Matchers with LocalSparkSession {
    "An RebalanceMapping" should "be parseable" in {
        val spec =
            """
              |mappings:
              |  m1:
        ...
        mapping shouldBe a[RebalanceMappingSpec]

        val instance = context.getMapping(MappingIdentifier("m1"))
        instance shouldBe a[RebalanceMapping]
