...
class DropMappingTest extends FlatSpec with Matchers with LocalSparkSession {
    ...
    it should "drop known columns" in {
        ...
        val outputDf = mapping.execute(executor, Map(MappingOutputIdentifier("df") -> inputDf))("default")
        outputDf.schema should be (expectedSchema)

        val outputSchema = mapping.describe(Map(MappingOutputIdentifier("df") -> ftypes.StructType.of(inputDf.schema)))("default")
