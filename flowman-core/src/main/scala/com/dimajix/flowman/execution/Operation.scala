...
object Operation {
    def run(name:String, description:Option[String]=None)(f: => Unit)(implicit executor: ExecutionContext): Operation = {
        AsyncOperation(
            name = name,
...
trait Operation {
    /**
     * Returns the user-specified name of the operation, or [None] if not specified.
     * @return
     */
    def name: String
    ...
    def addListener(listener:OperationListener) : Unit
    def removeListener(listener:OperationListener) : Unit
}


abstract class AbstractOperation extends Operation {
    protected val listeners = new OperationListenerBus()
