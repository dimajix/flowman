...
class SqlBuilder private(
  ...
  private def toSQL(node: LogicalPlan): String = node match {
    ...
    case Sort(orders, _, RepartitionByExpression(partitionExprs, child, _, _))
  ...
  private def scriptTransformationToSQL(plan: ScriptTransformation): String = {
    val ioSchema = plan.ioschema
    val inputRowFormatSQL = getRowFormatSQL(ioSchema.inputRowFormat, ioSchema.inputSerdeClass, ioSchema.inputSerdeProps, ioSchema.schemaLess)
        .getOrElse(throw new UnsupportedOperationException(
        s"unsupported row format ${plan.ioschema.inputRowFormat}"))
    val outputRowFormatSQL = getRowFormatSQL(ioSchema.outputRowFormat, ioSchema.outputSerdeClass, ioSchema.outputSerdeProps, ioSchema.schemaLess)
        .getOrElse(
      throw new UnsupportedOperationException(
    ...
    private def getRowFormatSQL(
        ...
        schemaLess: Boolean): Option[String] = {
  ...
  object Canonicalizer extends RuleExecutor[LogicalPlan] {
    ...
    object NormalizedAttribute extends Rule[LogicalPlan] {
      ...
      private def normalizeAttributes(plan: LogicalPlan): LogicalPlan = plan.transformExpressions {
        ...
        // Remove StaticInvoke for performing CHAR(n)/VARCHAR(n) read side padding (required since Spark 3.4)
        case s: StaticInvoke if s.objectName == "org.apache.spark.sql.catalyst.util.CharVarcharCodegenUtils" && s.functionName == "readSidePadding" =>
          s.arguments.head
      }
    }
