...
import com.dimajix.flowman.kernel.model.TargetOrder;
import com.dimajix.flowman.kernel.model.TargetQuery;
import com.dimajix.flowman.kernel.model.TargetState;
import com.dimajix.flowman.tools.ExecutionContext;
import com.dimajix.flowman.tools.rexec.Command;


public class SearchTargetHistoryCommand extends Command {
    ...
    String job = "";
    @Option(name = "-J", aliases={"--job-id"}, usage = "id of job run", metaVar = "<job_id>")
    String jobId = "";
    @Option(name = "-t", aliases= {"--target"}, usage = "name of job", metaVar = "<target>")
    String target = "";
    ...
    public Status execute(ExecutionContext context) {
        ...
        val targets = history.findTargets(query, Collections.singletonList(TargetOrder.BY_DATETIME), limit);

        val header = new String[]{"id", "job_id", "namespace", "project", "version", "target", "partitions", "phase", "status", "start", "end", "error"};
        val rows = targets.stream().map(SearchTargetHistoryCommand::toRow).collect(Collectors.toList());
        ConsoleUtils.showTable(header, rows);

        return Status.SUCCESS;
    }

    private static String[] toRow(TargetState state) {
        return new String[]{
            state.getId(),
            state.getJobId().orElse(""),
            state.getNamespace(),
            state.getProject(),
            state.getVersion(),
            state.getTarget(),
            state.getPartitions().entrySet().stream().map(e -> e.getKey() + "=" + e.getValue()).reduce((l,r) -> l + "," + r).orElse(""),
