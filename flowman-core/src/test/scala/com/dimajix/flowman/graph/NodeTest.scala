...
class NodeTest extends AnyFlatSpec with Matchers with MockFactory {
    "A Node" should "generate a nice tree" in {
        ...
        val srcRelationNode = RelationRef(srcRelation)
        val readMappingNode = MappingRef(readMapping)
        val mapping1Node = MappingRef(mapping1)
        val mapping2Node = MappingRef(mapping2)
        val mapping3Node = MappingRef(mapping3)
        val unionMappingNode = MappingRef(unionMapping)
        val targetNode = TargetRef(target)
        val tgtRelationNode = RelationRef(tgtRelation)

        tgtRelationNode.inEdges.append(WriteRelation(targetNode, tgtRelationNode))
        targetNode.inEdges.append(InputMapping(unionMappingNode, targetNode))
        unionMappingNode.inEdges.append(InputMapping(mapping1Node, unionMappingNode))
        unionMappingNode.inEdges.append(InputMapping(mapping2Node, unionMappingNode))
        unionMappingNode.inEdges.append(InputMapping(mapping3Node, unionMappingNode))
        mapping1Node.inEdges.append(InputMapping(readMappingNode, mapping1Node))
        mapping2Node.inEdges.append(InputMapping(readMappingNode, mapping2Node))
        mapping3Node.inEdges.append(InputMapping(readMappingNode, mapping3Node))
