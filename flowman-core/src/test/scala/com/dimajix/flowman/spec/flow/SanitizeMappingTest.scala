...
class SanitizeMappingTest extends FlatSpec with Matchers with LocalSparkSession {
    ...
    "A SanitizeMapping" should "be parseable" in {
        val spec =
            """
              |mappings:
              |  my_structure:
        ...
        mapping shouldBe an[SanitizeMapping]
    }

    it should "transform DataFrames correctly" in {
        val mapping = SanitizeMapping(
            "input_df",
            Map(
                "string" -> "int"
            )
