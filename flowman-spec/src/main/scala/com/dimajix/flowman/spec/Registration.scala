...
class RegistrationPluginListener extends PluginListener {
    override def pluginLoaded(plugin: Plugin, classLoader: ClassLoader): Unit = Registration.invalidate(classLoader)
}
