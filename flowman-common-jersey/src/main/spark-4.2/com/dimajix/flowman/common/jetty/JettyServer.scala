...
import org.sparkproject.jetty.ee10.servlet.{DefaultServlet, ServletContextHandler, ServletHolder}
...
class JettyServerBuilder {
    ...
    def build() : JettyServer = {
        ...
        handlers.setHandlers(List[Handler](handler).asJava)

        val bindUri = new URI("http", null, bindHost, bindPort, null, null, null)
        new JettyServer(bindUri, null, handlers, requestLog)
    }
}
...
class JettyServer private[jetty](uri: URI,
    ...
    private val server = createServer(uri, sslContextFactory, handler, requestLog)
