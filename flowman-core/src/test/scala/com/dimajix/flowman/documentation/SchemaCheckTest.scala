...
class SchemaCheckTest extends AnyFlatSpec with Matchers with MockFactory with LocalSparkSession {
    ...
    it should "support filter conditions" in {
        ...
        (() => valuesMapping.identifier).expects().atLeastOnce().returns(MappingIdentifier("excludes"))
        (() => valuesMapping.output).expects().atLeastOnce().returns(MappingOutputIdentifier("excludes"))
        (() => valuesMapping.outputs).expects().atLeastOnce().returns(Set("main"))
        (() => valuesMapping.inputs).expects().returns(Set.empty)
        (() => valuesMapping.context).expects().returns(context)
        (() => valuesMapping.broadcast).expects().returns(false)
        (() => valuesMapping.cache).expects().returns(StorageLevel.NONE)
        (() => valuesMapping.checkpoint).expects().returns(false)
    ...
    "A ForeignKeySchemaCheck" should "work" in {
        ...
        (() => mapping.context).expects().returns(context)
        (() => mapping.inputs).expects().returns(Set())
        (() => mapping.outputs).expects().atLeastOnce().returns(Set("main"))
        (() => mapping.broadcast).expects().returns(false)
        (() => mapping.cache).expects().returns(StorageLevel.NONE)
        (() => mapping.checkpoint).expects().returns(false)
        (() => mapping.identifier).expects().returns(MappingIdentifier("project/mapping"))
