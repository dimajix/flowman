...
class NodeTest extends AnyFlatSpec with Matchers with MockFactory {
    "A Node" should "generate a nice tree" in {
        ...
        val readMappingNode = MappingRef(2, readMapping, Seq(MappingOutput(3, null, "main")))
        val mapping1Node = MappingRef(4, mapping1, Seq(MappingOutput(5, null, "main")))
        val mapping2Node = MappingRef(6, mapping2, Seq(MappingOutput(7, null, "main")))
        val mapping3Node = MappingRef(8, mapping3, Seq(MappingOutput(9, null, "main")))
        val unionMappingNode = MappingRef(10, unionMapping, Seq(MappingOutput(11, null, "main")))
        val targetNode = TargetRef(12, target, Phase.BUILD)
        val tgtRelationNode = RelationRef(13, tgtRelation)
